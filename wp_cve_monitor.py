...
WP_ORG_VULN_API = "https://api.wordpress.org/plugins/vulnerabilities/v1/"
...
# ─────────────────────────────────────────────
# CVE SOURCE 1: WordPress.org Plugin Vulnerabilities API
# ─────────────────────────────────────────────
def fetch_wporg_vulnerabilities() -> list:
    """Fetch recent vulnerabilities from WordPress.org plugin API."""
    vulns = []
    try:
        data = http_get_json(
            f"{WP_ORG_VULN_API}?per_page=20",
            headers={"User-Agent": "CodeVigilant-CVE-Monitor/1.0"}
        )
        if data and isinstance(data, list):
            for v in data:
                cve_id = v.get("id", "")
                plugin_slug = v.get("name", "")
                title = v.get("title", "")
                ...
                vulns.append({
                    ...
                    "url": v.get("link", f"https://wordpress.org/plugins/{plugin_slug}/"),
                })
        print(f"[INFO] WordPress.org API: {len(vulns)} vulnerabilities found")
    except Exception as e:
...
def extract_plugin_from_cve_desc(desc: str) -> Optional[str]:
    ...
    patterns = [
        r'(?:plugin|theme)\s+["\']?([a-z0-9_-]+)["\']?',
        r'WordPress\s+(?:plugin|theme)\s+["\']?([a-z0-9_-]+)["\']?',
        r'(?:vulnerability|vulnerabilities)\s+in\s+["\']?([a-z0-9_-]+)["\']?',
        r'(?:affects|affected)\s+["\']?([a-z0-9_-]+)["\']?',
    ]
...
def generate_semgrep_rule(vuln: dict, plugin_dir: Path) -> Optional[Path]:
    ...
    rule_templates = {
        ...
        "xss": {
            ...
            "message": f"Potential XSS in {plugin_slug} ({cve_id}): unescaped output",
            "severity": "WARNING",
        ...
        "ssrf": {
            ...
            "pattern": "- pattern: file_get_contents(...)",
            "message": f"SSRF in {plugin_slug} ({cve_id}): potential server-side request forgery",
            "severity": "ERROR",
    ...
    if not template:
        ...
        template = {
            ...
            "message": f"Vulnerability in {plugin_slug} ({cve_id}): {vuln_type}",
            "severity": "WARNING",
...
def create_rule_pr(vulns: list, rule_files: list) -> Optional[int]:
    ...
    # Create branch
    branch_name = f"feat/cve-rules-{datetime.utcnow().strftime('%Y%m%d-%H%M')}"
    subprocess.run(
...
# ─────────────────────────────────────────────
def main():
    print(f"[INFO] WordPress CVE Monitor starting at {datetime.utcnow().isoformat()}")
    state = load_state()
    processed = set(state.get("processed_cves", []))

    # 1. Fetch CVEs from all sources
    all_vulns = []
    all_vulns.extend(fetch_wporg_vulnerabilities())
